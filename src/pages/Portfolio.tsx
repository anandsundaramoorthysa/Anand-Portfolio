...
export default function Projects() {
  ...
  const handleShare = (project) => {
    const shareText = `Check out my project: ${project.title} - ${project.description}. ${project.demo ? `Live Demo: ${project.demo}` : ''} ${project.github ? `GitHub: ${project.github}` : ''}`;
          ...
          >
            ...
            <div className="mt-6 flex justify-center">
              <div className="bg-accent/10 rounded-full flex items-center overflow-hidden">
                {/* {["all", "web development", "app development", "python package", "others"].map(category => { */}
              ...
              >
                <Card className="card-hover h-full flex flex-col overflow-hidden">
                  <div className="relative h-48 w-full overflow-hidden">
                    ...
                    />
                    {project.completionDate && ( // Check if completionDate exists
                      <div className="absolute top-2 right-2 bg-primary text-primary-foreground text-xs px-2 py-1 rounded-full">
                        {project.completionDate} {/* Display the completion date */}
                      </div>
                    )}
                  </div>
